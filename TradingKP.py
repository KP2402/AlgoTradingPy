# -*- coding: utf-8 -*-
"""
Created on Thu Nov 23 23:32:38 2017
...
df['closeAsk1']=df['closeAsk']
df['closeAsk1Shift']= df['closeAsk'].shift(1)
df['returns'] = np.log(df['closeAsk'] / df['closeAsk'].shift(1))  # 12
#df['momentum']=df['returns'].rolling(15)

cols = []  # 13

for momentum in [15, 30, 60, 120]:  # 14
    col = 'position_%s' % momentum  # 15
    colMean = 'Mean_%s' % momentum 
    df[colMean] = df['returns'].rolling(momentum).mean()
